...
查询友盟应用性能监控（U-APM）的崩溃详情数据、自定义异常上报与卡顿日志，包括 case 排名、日志实例列表、完整日志详情（设备 + 行为 + 地理位置），共 **4 个只读查询接口**。
...
- 本 Skill 的 4 个接口均为 `GET` 方法
- 参数 JSON 中所有字段均为必填（除非特别标注）

## 关键概念

本 Skill 涉及 3 个 ID + 1 个文件资源，调用前请先理清它们的层级与边界，避免传错参数。

### errorId vs logId vs rawLog
...
| `rawLog`（原始日志 zip） | `GetRawLogSearch` 返回的 `Data` URL | **OSS 签名下载链接**：含完整堆栈、日志上下文、用户行为流水的 zip 包 | 单次崩溃的原始文件 | `.../UAPM_原始日志_xxx.zip?Signature=...` |
...
4 个接口共用同一组 `ErrorType` + `CrashType` 枚举，可组合关系如下：
...
### 下载类（1 个，返回签名 URL，需 curl 下载 + unzip 解压）

| Action | Endpoint | 关键入参 | 输出形态 |
|--------|----------|---------|----------|
| `GetRawLogSearch` | `/stat/GetRawLogSearch` | + ErrorId + LogId | OSS 签名 zip 下载 URL（15 天有效期） |
...
3. GetErrorLogDetail (ErrorId + LogId)
   → 展示完整设备信息、行为轨迹、地理位置
4. (可选) GetRawLogSearch (ErrorId + LogId)
...
3. GetErrorLogDetail (ErrorType="pa", CrashType="", ErrorId + LogId)
   → 展示完整设备信息、行为轨迹、地理位置
4. (可选) GetRawLogSearch (ErrorType="pa", CrashType="", ErrorId + LogId)
   → 获取 zip URL → curl 下载 → 自动解压 → 告知文件路径
```
...
- **原始日志过期**：返回 code=310，OSS 文件仅保留 15 天
- **原始日志功能关闭**：返回 code=311，需应用管理员在后台开启
...
- `pagesBehavior` 字段为 **JSON 字符串**（非对象），使用时需先 `JSON.parse` / `json.loads` 解析，解析后结构如：
  ...
  ```
- GetRawLogSearch 返回的是 **zip 压缩包** URL，下载后需 `unzip` 解压
- 所有接口均为 `GET` 方法
